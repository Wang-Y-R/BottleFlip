...
bool Game() {
    ...
    while (true) {
        ...
        if (Keyboard[SDL_SCANCODE_SPACE] == true) {//蓄力开跳
            ...
            switch (Game_Check()) {
                ...
                case 1://跳到下一个方块了！先生成下一个方块,再根据玩家有没有继续跳检测要不要执行当前方块的事件
                    ...
                    while (Keyboard[SDL_SCANCODE_SPACE]==false && waitEnd-waitStart <= 3000) {
                        if (!Input_GetEvent()) {
                            Game_Quit();
                            return false;
                        }
                    ...
                    if (waitEnd - waitStart <= 3000) break;
                    //检测上一个方块类型，进行相应的交互
        ...
        if (Keyboard[SDL_SCANCODE_ESCAPE] == true) {//ESC退出
            break;
        }
        Game_DrawAll();
...
static void Game_DrawText() {
    ...
    Display_DrawText(WINDOW_WIDTH/8, 0, Score, 0, 0, &Color.White, ScoreFont);
    Display_DrawText(WINDOW_WIDTH/2,0,Combo,0,0,&Color.White,ComboFont);
}
...
static bool Game_TextAppearAnimation(void) {
    Input_Clear();
    float speed = 5,scoreX = -300,comboX = WINDOW_WIDTH;
    while (scoreX < WINDOW_WIDTH/8 || comboX > WINDOW_WIDTH/2) {
        if (!Input_GetEvent()) return false;
        if (scoreX < WINDOW_WIDTH/8) {
            scoreX+=speed;
        }
        if (comboX > WINDOW_HEIGHT/2) {
            comboX -= speed;
        }
        Game_DrawBlockAndBackground(BlockHead.nextBlock);
        Player_Draw(1);
        ...
        Display_DrawText(comboX, 0, Combo, 0, 0, &Color.White, ScoreFont);
        SDL_RenderPresent(Renderer);
        SDL_Delay(5);
        if (Mouse.status==1) break;
    }
...
static void Game_CreateNextBlock(bool first) {
    ...
    } while ((float)Distance < BlockHead.nextBlock->w ||
    (Direction == left && ( (float)Distance+BlockSize[Kind])>=Player.centerX ) ||
    (Direction == right && ( (float)Distance+Player.centerX+BlockSize[Kind]) > WINDOW_WIDTH ));
    Block_Create(Player.centerX,Player.centerY,(float)Distance,Kind,Direction);
}
...
static bool Game_GatherStrength(void) {
    float strength = 0,speed = 1,playerXDistance = Player.centerX - BlockHead.nextBlock->nextBlock->centerX,playerYDistance = Player.centerY - BlockHead.nextBlock->nextBlock->centerY;
    Mix_PlayChannel(6,Music_rising,0);
    while (Keyboard[SDL_SCANCODE_SPACE]==true) {
        ...
        SDL_Color color = {(int) (strength / MaxStrength * 255),(int) (((MaxStrength - strength) * 1.0 / MaxStrength) * 255), 0, 255};
        Display_FillFRect(&strengthFRect,&color);
        SDL_RenderPresent(Renderer);
        uint64_t end = SDL_GetTicks64();
        uint64_t pause = end - start;
        SDL_Delay(pause < 2 ? 2 - pause : 0);
    }
    Mix_Pause(6);
    //恢复数据
...
static void Game_AddScoreAnimation(int score) {
    while (score >= 0) {
        ...
        char Add[20] = {"(0.0)+"},number[10]={'\0'};
        strcat(Add, itoa(score, number, 10));
        Player.score++;
        Game_DrawAll();
        Display_DrawText(WINDOW_WIDTH/8, 70, Add, 0, 0, &Color.White, ScoreFont);
        score--;
        SDL_RenderPresent(Renderer);
        SDL_Delay(100);
    }
...
static void Game_Scroll(void) {
    ...
    int delay = 20;
    for (; SDL_fabsf(Player.centerX - WINDOW_HEIGHT/2) >= 5.0f;) {
        Input_GetEvent();
        ...
        while (cur->nextBlock != NULL) {
            ...
            if ( (next->centerX+next->w/2 < 0) || (next->centerX-next->w/2 > WINDOW_WIDTH) || (next->centerY-next->w*2/3 > WINDOW_HEIGHT)) {
